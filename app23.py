...
def load_data(start, end):
    try:
        ...
        if 'Adj Close' in nifty.columns:
            ...
            prices = nifty[['Close']].rename(columns={'Close': 'Price'})
        else:
            st.error(f"Available columns: {nifty.columns.tolist()}")
            st.stop()
            
        if 'Adj Close' in rates.columns:
            rates = rates[['Adj Close']].rename(columns={'Adj Close': 'Rate'})
        elif 'Close' in rates.columns:
            rates = rates[['Close']].rename(columns={'Close': 'Rate'})
        else:
            st.error(f"Available rate columns: {rates.columns.tolist()}")
            st.stop()
        
        return prices.merge(rates, left_index=True, right_index=True, how='inner').dropna()
